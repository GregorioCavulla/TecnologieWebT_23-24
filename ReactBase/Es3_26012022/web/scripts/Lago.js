...
class Lago extends React.Component {
  	...
  	click(e){
		  ...
		  if( altraX >=0 && altraY>=0 && altraX<=(parseInt(this.props.Larghezza)-1) && altraY<=(parseInt(this.props.Lunghezza)-1)){
			  if(document.getElementById(`${altraY}-${altraX}`).innerText == "")punti += parseInt(pesci[altraY][altraX]);
			  document.getElementById(`${altraY}-${altraX}`).innerText = pesci[altraY][altraX];
		  }
		...
		if(this.props.Gen == 1){
		 	 ...
		 	 for(let i = 0; i<this.props.Lunghezza; i++){
			 	 ...
			 	 for(let j=0; j<this.props.Larghezza; j++){
					  righa.push(<button id={`${i}-${j}`} ref={this.buttonRef} style={{width: '30px',height: '30px'
					  ,margin: '2px',fontSize: '14px'
