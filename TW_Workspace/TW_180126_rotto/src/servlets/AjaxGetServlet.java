...
public class AjaxGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("Sono nella servlet AjaxGet");
		ServerData serverData = ServerData.getServerData();
		List<Conteggio> conteggi = new ArrayList<Conteggio>();
		int totaleOccorrenze = 0;
		int totaleUrl = serverData.getTotaleUrl();
		int isTuttiFiniti=0;
		for (Conteggio c : conteggi) {
			totaleOccorrenze += c.getOccorrenze();
			if(c.isFinito()) {
				isTuttiFiniti+=1;
			}
		}
		if(isTuttiFiniti==totaleUrl) {

		// Convertire la lista degli utenti in formato JSON
		Gson gson = new Gson();
		String json = gson.toJson(totaleOccorrenze);
