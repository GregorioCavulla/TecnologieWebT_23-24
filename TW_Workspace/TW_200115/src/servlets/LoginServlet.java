...
public class LoginServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		for (int i = 0; i < 6; i++) {
			serverData.creaGruppo(""+i);
			for(Gruppo g : serverData.getGruppi()) {
				System.out.println("gruppo: "+g.getName());
			}
		...
		serverData.getUtente("cecco").setPasswordDate(LocalDate.now().minusDays(100));

		// Crea e aggiungi l'utente "andre"
		serverData.creaUtente("andre", "andrepw","1");
		serverData.getUtente("andre").setPasswordDate(LocalDate.now().minusDays(100));

		// Crea e aggiungi l'utente "andre"
		serverData.creaUtente("pietro", "pietropw","1");
		serverData.getUtente("pietro").setPasswordDate(LocalDate.now().minusDays(100));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if ("Registrazione".equals(action)) {
			...
			if (!serverData.creaUtente(username, password, gruppo)) {
				...
				if (serverData.getGruppoByUtente(utente) != null) {
					serverData.getGruppoByUtente(utente).addUtente(utente);
				} else {
					System.out
							.println("strano, l'utente non appartiene a nessun gruppo, non dovrebbe essere possibile");
				}
				if (serverData.getGruppoByUtente(utente).arePasswordOld()) {
					response.sendRedirect("./pages/resetpassword.jsp");
				} else {
