...
function handleMessage(ws, message) {
  try {
    ...
    if (content && typeof content === "string" && !/\d/.test(content)) {
      activeSessions[groupId].messages.push({
        sessionId,
        content,
        timestamp: Date.now(),
      });
      ...
      wss.clients.forEach((client) => {
        if (client.readyState === WebSocket.OPEN) {
          client.send(
            JSON.stringify({ groupId, message: { sessionId, content } })
          );
        }
      });
    }
...
function handleDisconnect(ws) {
  ...
  for (const id in activeSessions) {
    if (activeSessions[id].users[ws.sessionId]) {
      groupId = id;
      break;
    }
  }
  ...
  if (groupId) {
    delete activeSessions[groupId].users[ws.sessionId];

    // If the user was the last one in the group, delete the group
    if (!Object.keys(activeSessions[groupId].users).length) {
